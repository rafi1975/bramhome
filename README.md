...
| Tool | What it does |
| --- | --- |
| ESPHome add-on in Home Assistant | Builds this dongle's firmware from the YAML |
| [web.esphome.io](https://web.esphome.io) in Chrome or Edge | Writes that firmware to the dongle over USB |
...
4. In the ESPHome add-on, click **Install**, then **Manual download**, and choose **Factory format**. Save the `.bin` file. Compiling takes several minutes.
5. On your computer, open [web.esphome.io](https://web.esphome.io) in Chrome or Edge.
6. Put the dongle in flash mode: with it unplugged, hold the button next to the USB-C port, then plug the cable into the computer. The LED stays off in this mode.
7. On the website, click **Connect**, choose the dongle's serial port, click **Install**, and select the factory `.bin` from step 4.
8. When the site says the install succeeded, unplug USB, plug the dongle into the meter, and wait for the LED to turn green.
...
If you already clicked **Prepare for first use** and the dongle joined your Wi-Fi, skip the website install. Paste this YAML into the ESPHome add-on and choose **Wirelessly** when you click Install. Later updates use the network the same way.
